...
// Error Boundary
// ============================================
class ErrorBoundary extends React.Component<{children: React.ReactNode; step: string; result: unknown}, {hasError: boolean; error: string}> {
  constructor(props: {children: React.ReactNode; step: string; result: unknown}) {
    super(props)
  ...
  componentDidCatch(e: Error, info: React.ErrorInfo) {
    console.error('[ErrorBoundary] caught:', e.message, info.componentStack?.slice(0, 200))
  }
  render() {
    if (this.state.hasError) {
      return (
        <main className="max-w-2xl mx-auto px-6 pt-16 pb-12">
          <div className="rounded-2xl p-6 text-center" style={{background:'rgba(44,74,62,0.06)',border:'1px solid rgba(44,74,62,0.15)'}}>
            <p className="text-2xl mb-3">⚠️</p>
            <p className="text-sm font-medium" style={{color:'#2C4A3E'}}>頁面發生錯誤</p>
            <p className="text-xs text-stone-500 mt-2">結果顯示失敗，請重新整理頁面再試</p>
            <p className="text-xs text-stone-400 mt-1 font-mono">{this.state.error}</p>
          </div>
        </main>
      )
    }
